...
async function assertEncryptedJson(p) {
  ...
  if (!looksEncrypted(j)) {
    throw new Error("Encrypted JSON missing fields (accepted keys: enc|ciphertext|ct|data|box; nonce|n|header.nonce; epk|ephemeralPublicKey|header.epk|header.kid; or sealed).");
  }
}`);
  }
...
// Heuristic acceptance for sealed/box payloads from various toolchains
function looksEncrypted(j) {
  const hasCT =
    j.enc || j.ciphertext || j.ciphertextB64 || j.ct || j.cipher || j.box || j.payload || j.data;
  const hasNonce =
    j.nonce || j.nonceB64 || j.n || (j.header && (j.header.nonce || j.header.iv));
  const hasEpk =
    j.epk || j.epkB64 || j.ephemeralPublicKey || (j.header && (j.header.epk || j.header.kid));
  const isSealed = j.sealed || j.type === "sealedbox" || j.type === "sealed";
  return !!((hasCT && (hasNonce || isSealed)) && (hasEpk || isSealed));
}
