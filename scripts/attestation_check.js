...
function requireField(o,p){ const ok = p.split(".").reduce((x,k)=> (x&&k in x)? x[k] : null, o); if(ok===null){ throw new Error("Missing field: "+p); } }
["provider","epochId","enclave.tee","enclave.mrenclave","enclave.mrsigner","enclave.productId","enclave.svn","nonce","issuedAt","evidenceCid","signature"].forEach(f=>requireField(att,f));
const ml = (allow.measurementAllowlist||[]).some(m =>
...
if(!ml){ console.error("Attestation measurement not in allowlist"); process.exit(3); }
