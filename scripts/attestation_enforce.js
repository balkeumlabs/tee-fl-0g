...
const enforced = String(process.env.ATTESTATION_ENFORCED ?? "true").toLowerCase() !== "false";
...
try{
  ...
  const allowlistPath = argv.allowlist || process.env.ATT_ALLOWLIST_PATH || "./attestation_allowlist.json";
  ...
  if(!enforced){
    console.log(JSON.stringify({ok:true,reason:"bypassed",enforced:false,measurement:meas}));
    process.exit(0);
  }
  if(!meas){ console.log(JSON.stringify({ok:false,reason:"measurement-missing",measurement:""})); process.exit(2); }
  if(!list.includes(meas)){ console.log(JSON.stringify({ok:false,reason:"measurement-not-allowed",measurement:meas,allowlistPath})); process.exit(3); }
