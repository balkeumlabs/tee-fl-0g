...
app.get('/api/epoch/latest', asyncHandler(async (req, res) => {
    try {
        ...
        // If demo mode is enabled but no demo data exists, fall through to show blockchain data
        // This provides better UX - dashboard still works even if demo training hasn't been run yet
        if (demoMode.enabled && !demoMode.epochData && !forceBlockchain) {
